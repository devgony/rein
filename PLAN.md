...
도구 이름은 **reins**(고삐)다. harness를 채운 말에게 명령을 전달하는 인터페이스라는 뜻으로, LLM에게 작업을 지시하는 이 도구의 성격을 담는다. 영어에서 고삐는 좌우 한 쌍으로 쓰여 명사로는 복수형 reins가 표준이다 ("take the reins").
...
`state.json`이 깨지거나 사라져도 `reins doctor`가 task 파일 스캔으로 재생성할 수 있어야 한다.
...
`.reins/`는 기본적으로 git에 포함하지 않는다. 도구가 `init` 시 `.git/info/exclude`에 추가한다. 팀 차원의 공유 템플릿이나 skill만 repo에 포함할지는 별도 옵션으로 둔다.
...
```sh
reins new "settings cleanup"
reins open settings-cleanup
```

결과:

- `.reins/inbox/settings-cleanup.md` 생성
- `$EDITOR`로 파일 열기
- 아직 GitHub에는 공개하지 않음
...
```sh
reins publish settings-cleanup
```
...
- label은 `reins` 하나만 쓴다. 상태(inbox/active)는 로컬에서만 추적한다.
- local frontmatter에 `github_issue` 기록

issue body 형태:

```markdown
<!-- reins:begin task-20260612-settings-cleanup -->
...
```sh
reins start settings-cleanup
```

결과:

- 문서를 `.reins/active/`로 이동
...
skill은 `reins current --path`로 현재 task 문서를 찾는다.

LLM은 다음 규칙을 따른다.

- unchecked task만 수행한다.
- 상태 변경은 Markdown 직접 편집이 아니라 CLI mutation 명령으로 한다.
  - `reins check <item-id>`: 구현과 검증이 끝난 항목 체크
  - `reins log "<text>"`: Agent Log에 append
  - `reins fail <item-id> --reason "<text>"`: blocker 기록
- 완료하지 않은 항목은 체크하지 않는다.
- 각 task 완료 후 관련 검증을 수행한다.
- 필요하면 `reins push`로 PR body나 issue body를 갱신한다.
...
- PR body 전체를 덮어쓰지 않는다. `reins:begin`/`reins:end` 마커 사이의 managed section만 갱신하고, 마커 바깥에 사람이 쓴 내용은 보존한다.
...
```sh
reins done settings-cleanup
```

결과:

- 문서를 `.reins/done/YYYY-MM/`로 이동
- 관련 issue를 닫거나 완료 코멘트를 남김
- PR body의 managed section에 최종 summary와 validation 갱신

cancel의 경우:

- 문서를 `.reins/canceled/`로 이동
...
```text
reins init
reins new <title> [--shared]
reins list [--status inbox|active|done|canceled]
reins open [task]              # 인자 없으면 fuzzy picker
reins current [task] [--path]  # 조회 / 전환
reins start <task> [--branch] [--draft-pr]

# LLM-safe mutation (skill이 사용)
reins check <item-id>
reins uncheck <item-id>
reins log <text>
reins fail <item-id> --reason <text>

reins publish <task>
reins pull-inbox
reins pull
reins push [--resolved]
reins attach-issue <number>
reins attach-pr <number>
reins done [task]
reins cancel [task]
reins doctor                   # state.json 재생성, 무결성 검사
reins status
reins ui                       # TUI dashboard (Phase 5)
```
...
- 소유권 판정은 label이 아니라 body의 `reins:begin` 마커로 한다. label은 검색 필터일 뿐이다.
...
```json
{
  ...
  "tasks": {
    "task-20260612-settings-cleanup": {
      "path": ".reins/active/settings-cleanup.md",
...
- `state.json`이 깨지면 `reins doctor`가 task 파일 스캔으로 재생성한다. synced hash는 유실 시 다음 sync에서 conflict로 안전하게 fallback한다.
...
- conflict 시 `.reins/conflicts/`에 local/remote 백업을 저장한다.
- 사용자가 해결한 뒤 `reins push --resolved`를 실행한다.
...
```markdown
---
description: Run the current LLM task document, implement unchecked tasks, update status via reins commands, and append execution notes.
disable-model-invocation: true
---

Run `reins current --path` to find the active task document, then read it.

Rules:

1. Execute only unchecked tasks.
2. Never edit checkboxes or Agent Log in the Markdown directly. Use:
   - `reins check <item-id>` after a task is implemented and verified
   - `reins log "<text>"` to append a concise entry after each completed task
   - `reins fail <item-id> --reason "<text>"` when blocked
3. Preserve `<!-- task:... -->` ID comments when editing other sections.
4. Run relevant tests before checking validation items.
5. If a PR is attached, run `reins push` when finished.
```
...
- `reins open`을 인자 없이 실행하면 내장 fuzzy picker로 task를 고른다. nucleo(helix의 fuzzy matcher) 기반이면 외부 의존성이 없다.
- 파일이 평범한 Markdown 디렉토리이므로 Obsidian vault나 VS Code workspace로 열어 보는 것도 그대로 동작한다.

### Phase 5: ratatui TUI dashboard

`reins ui`로 진입하는 ratatui 기반 dashboard.
...
- `reins` label 관리 (label은 검색 필터, 소유권은 body 마커)
