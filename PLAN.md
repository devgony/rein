...
`state.json`이 깨지거나 사라져도 `rein doctor`가 task 파일 스캔으로 재생성할 수 있어야 한다.
...
`.rein/`는 기본적으로 git에 포함하지 않는다. 도구가 `init` 시 `.git/info/exclude`에 추가한다. 팀 차원의 공유 템플릿이나 skill만 repo에 포함할지는 별도 옵션으로 둔다.
...
- `.rein/inbox/settings-cleanup.md` 생성
- `$EDITOR`로 파일 열기
- 아직 GitHub에는 공개하지 않음
...
rein start <task> [--branch] [--draft-pr]
...
rein done [task]
rein cancel [task]
rein doctor                   # state.json 재생성, 무결성 검사
rein status
...
```json
{
  ...
  "tasks": {
    "task-20260612-settings-cleanup": {
      "path": ".rein/active/settings-cleanup.md",
...
- `state.json`이 깨지면 `rein doctor`가 task 파일 스캔으로 재생성한다. synced hash는 유실 시 다음 sync에서 conflict로 안전하게 fallback한다.
...
- conflict 시 `.rein/conflicts/`에 local/remote 백업을 저장한다.
