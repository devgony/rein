...
rein store는 worktree 영향을 받지 않도록 프로젝트 안이 아니라 repo별 홈 경로에 둔다. 이하 예시의 `<store>`는 이 경로를 가리킨다.
...
- `<repo-key>`는 `git rev-parse --git-common-dir` 기반으로 만든다. 한 repo의 모든 worktree가 같은 common-dir을 공유하므로 같은 store로 수렴한다. 클론이 둘이면 자연히 분리된다.
- 어느 worktree에서 `rein`을 실행하든 cwd에서 repo를 찾아 common-dir 키 → store를 해석한다.
...
- 사람이 에디터나 GitHub UI에서 ID 없이 추가한 항목은 도구가 `new`/`issue`/`push`/`pull` 시점에 자동으로 ID를 부여한다.
...
- worktree가 있으면 `git worktree remove` (포인터도 함께 사라짐)
...
rein current [task] [--path]  # 조회 / 전환
rein start <task> [--worktree] [--branch] [--draft-pr]

# LLM-safe mutation (skill이 사용)
rein check <item-id>
rein uncheck <item-id>
rein log <text>
rein fail <item-id> --reason <text>
...
- `git rev-parse --git-common-dir` → 모든 worktree가 공유하는 키 → 같은 task 풀(inbox/active). 브랜치가 달라도 common-dir은 같다.
- `git rev-parse --git-dir` → worktree마다 다름 → 어느 task인지.

이 두 git 질의가 "공유 풀"과 "내 task"를 분리한다.
...
- store 구조 생성 (repo별 홈 경로, `git-common-dir` 키, `REIN_ROOT` override)
...
- 두 클론(서로 다른 common-dir)이 같은 inbox를 공유하길 원할 때의 처리
